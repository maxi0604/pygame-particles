...
def get_color(x):
    ...
    if x <= 0.5:
        rres, gres, bres = llerp(rcld, rmid, 2 * x), llerp(gcld, gmid, 2 * x), llerp(bcld, bmid, 2 * x)
    else:
        rres, gres, bres = llerp(rmid, rhot, 2 * x - 1), llerp(gmid, ghot, 2 * x - 1), llerp(bmid, bhot, 2 * x - 1)
